...
> 方差来自**应用框架的随机失败**（8 步工具迭代上限 + 新模式「原文直吐」）：D1 命中率已退化为 **1/10**。
...
> 77 次采样显示，**快模型的 agent 更容易触顶框架 8 步工具迭代上限**——
> D1 的命中率从「命中」退化为 **1/10**，D2 **5/10**，还出现了新模式「原文直吐」（答案直接吐检索片段）。
> 所以准确的说法是：**延迟与质量的 trade-off 比预期宽松，但稳定性的代价被低估了。**
> 这笔账已列入 P0（见 `docs/复盘报告.md` §7 第 4 项）。
