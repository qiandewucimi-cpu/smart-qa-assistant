...
- **英文词条忽略大小写 + 加词边界**：OCR 输出大小写不可控（截图里是 `某外贸公司`，文档里是 `某外贸公司`），大小写敏感会漏；而英文词不加边界又会误伤（`海外工厂` 命中 `ORIENTATION`）。
- **能锚定上下文就别用裸正则**：BIC 码字面格式（8/11 位大写串）会误伤 `SHANGHAI`、`CHOCOLAT`、`SUBCLASS`、`SOURCING`，所以改成必须跟在 `SWIFT Code：` 之后。
...
详细结论见 `eval/评测报告.md` 与 `eval/评测集.md`；质量门槛与现状对标见 `docs/质量门槛对标.md`。
