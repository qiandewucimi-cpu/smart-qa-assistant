...
> 基于 LLM-Wiki 的企业知识库问答原型：把散落的工作文档清洗、脱敏、分批导入知识库，通过对话式检索回答业务问题，最终以**飞书机器人**作为员工问答入口，并用评测集持续验证问答质量。
...
项目来自真实工作中的知识获取问题：培训资料、会议纪要和业务截图分散在不同文件中，靠人工翻找效率低，也难以复用。我负责识别这一问题，定义知识范围、脱敏边界和验收问题，使用真实业务问法反复测试正常问答、多轮追问与语料外拒答，并根据失败结果推动术语对照、模型配置和交互入口迭代。

代码和脚本在 AI 开发工具辅助下完成；我的核心贡献是**业务问题发现、需求与边界定义、测试验收、失败反馈和持续迭代**。本仓库提供的是可复现原型与评测证据，不等同于已经完成企业级生产部署。
...
- **混合检索**：当前运行实例使用关键词 + 知识图谱两路融合；向量一路（`embedding-3`，764/817 页索引已建）
