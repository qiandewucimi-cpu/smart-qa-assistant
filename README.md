...
> 方差来自**应用框架的随机失败**（8 步工具迭代上限 + 新模式「原文直吐」）：D1 在 10 次采样里只命中 **1/10**
> （⚠️ **不是"退化"**——此前只测过它 1 次，没有基线；且"换模型导致"这个归因已因**无对照**被撤回）。
> **本项目对外一律报「18~19/21（±1~2 题），随框架随机失败波动」，并附复跑次数，不报单一数字。**
...
>
> ⚠️⚠️ **顺带发现的最要命一条**：对照臂这次 D1 是 **3/5**，v1.1 同配置测的是 **1/10**——
> **什么都没改，差 50pp**。这个系统的测量噪声**大于我调过的任何一个旋钮的效应量**，
> 所以下一步不是继续调参，而是**扩样本到 ≥60 题 + 每题 ≥5 次基线采样**。
>
> 最朴素的解释才是真的：**D1 本来就一直在 8 步上限边缘，v1.0 那次命中就是 1/10 里的那一次**——
> 不是它退化了，是**我们从来没测过它的基线稳定性，却默认它稳定**。
> 所以这件事的正确记法是「**样本量不足 + 缺基线采样**」，而不是「模型选型的代价」。
