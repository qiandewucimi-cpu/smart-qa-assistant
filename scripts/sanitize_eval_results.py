...
问题出在**开启向量检索之后**：chat API 的 `references` 不再只返回关键词命中的页，
而是会把语义相近的**真实实体页**也带回来，于是它们的 `path` / `title` 就写进了
`eval/评测结果_raw*.json`——而 `eval/` 是**入库的**。

实测（2026-09-12）：v1.0 评测结果里出现了 `wiki/entities/purchase-order-<真实订单号>.md`、
`wiki/entities/<真实物流商>-LOGISTICS.md`，而上一轮（无向量检索）的产物是干净的。
即 **开向量检索扩大了外泄面**，必须加一道擦除闸门。
