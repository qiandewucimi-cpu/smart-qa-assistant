...
def main():
    ...
    if n_ok_samples:
        ...
        if not nfail_q:
            ...
            print('       ⚠️ 但这**不等于**「这些题天生需要很多步」——v1.3 §8 用干净基线重测 D1/D2，')
            print('          框架失败 0/20、步数中位 10（6~17）：步数是**当次运行条件**的函数，不是题的属性。')
    ...
    if steps:
        ...
        print('  ⚠️ 读法警告（v1.3 §8）：`limit` 失败的**定义**就是「把预算用满」→ 事件数必然 ≈27')
        print('     → **必然落进 24~28 桶**。所以「24~28 桶失败率高」是**同义反复**，')
        print('     不能倒推「该题天生需要 27 步」。它只说明：当次走了这么多步的，多半会失败。')
